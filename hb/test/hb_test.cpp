...
BOOST_AUTO_TEST_CASE(test_log) try {
  log_trace<<"log test "<<"ok";
  log_debug<<"log test "<<"ok";
  log_info<<"log test "<<"ok";
  log_warn<<"log test "<<"ok";
  log_error<<"log test "<<"ok";
  log_fatal<<"log test "<<"ok";
...
BOOST_AUTO_TEST_CASE(test_list) try {
    ...
    int from = 1, to = 3;
    if (from < 0 || from >= m_animtStepList.size() ||
        to < 0 || to >= m_animtStepList.size())
    {
        return ;
    }
...
BOOST_AUTO_TEST_CASE(test_md5) try {
  std::string source_str1("123455666");
...
-----END PUBLIC KEY-----)");
  ...
  std::string sig = base64_to_str(R"(KW6M4jqPpeSwmaBkL26ZIZklwjLp/NIasafn675EPX/w1p1L2VsgPixf+UjFcfBOMqxe8w3dzww1fTxbMNLUDY5fTRLO4u7HU0nqNTTCi4BQdH/GZ/c/kEInK6C0g8tcn0EX9NoUpqdNPzA6/lKDwK4xhJbvhL2s+ebzuLiYzXQ=)");
