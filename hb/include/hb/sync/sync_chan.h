...
namespace hb {
    namespace sycn {
        template <typename CHAN_VALUE> class sync_chan {
          public:
            ...
            friend void operator>>(sync_chan<CHAN_VALUE> &c, CHAN_VALUE &data) { c.m_read(data); }

            friend void operator>>(const CHAN_VALUE &data, sync_chan<CHAN_VALUE> &c) {
                c.m_send(data);
            }
            friend void operator<<(CHAN_VALUE &data, sync_chan<CHAN_VALUE> &c) { c.m_read(data); }
            ...
            inline void wait(const int &num = 1) {
                ...
                do {
                    ...
                    if (rv == -1) {
                        ...
                        if (time_out_check) {
                            hb_throw(
                                hb_sync_exception().msg("sync_chan read sign time out error!"));
                        } else
                            continue;
                    else
