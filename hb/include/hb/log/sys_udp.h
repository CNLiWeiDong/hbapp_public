...
namespace hb {
    namespace log {
        ...
        class udp_skin_factory : public logging::sink_factory<char> {
          public:
            ...
            boost::shared_ptr<sinks::sink> create_sink(settings_section const& settings) {
                ...
                if (boost::optional<std::string> param = settings["IP"]) ip = param.get();
                std::string port = "8888";
                if (boost::optional<std::string> param = settings["Port"]) port = param.get();
                boost::shared_ptr<sinks::syslog_backend> backend(new sinks::syslog_backend(
                    ...
                    keywords::use_impl
                    = sinks::syslog::udp_socket_based /*< the built-in socket-based implementation
                                                         should be used >*/
                    ));
                backend->set_target_address(ip, std::stoi(port));

                // Create the sink
                // boost::shared_ptr< udp_skin_backend > backend = boost::make_shared<
                // udp_skin_backend >(ip, port);
