...
#define SET_CUSTOM_ATTRIBUTE(VALUE_TYPE, KEY, VALUE) \
    boost::log::attribute_cast<boost::log::attributes::mutable_constant<VALUE_TYPE>>(hb::log::general_logger::get().get_attributes()[KEY]).set(VALUE);
...
({                                                                              \
    ...
    log_fatal<<sum<<" detail:"<<error_str;                                      \
    error_str;                                                                  \
})
