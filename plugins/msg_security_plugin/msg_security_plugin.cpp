...
namespace hb {
    namespace plugin {
        ...
        void msg_security_plugin::set_program_options(options_description &cli,
            ...
            cfg.add_options()
                ("http-security-rsa-private-key", boost::program_options::value<string>()->default_value("http-security-rsa-private-key.pem"), "rsa private key use to encrypt http body!")
                ("http-security-rsa-private-pass", boost::program_options::value<string>()->default_value(""), "the password of rsa private key!")
                ("http-security-rsa-public-key", boost::program_options::value<string>()->default_value("http-security-rsa-public-key.pem"),"rsa public key use to decrypt http body!")
            ;
        }
        void msg_security_plugin::plugin_initialize(const variables_map &options) {
            ...
            hb_try {
                ...
                my->http_security_private_key(hb::crypto::read_rsa_pem_pri(private_key_file.c_str(), options.at("http-security-rsa-private-pass").as<string>()));
