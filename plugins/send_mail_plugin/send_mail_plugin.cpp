...
namespace hb {
    namespace plugin {
        static appbase::abstract_plugin& _send_mail_plugin
            = app().register_plugin<send_mail_plugin>();
        send_mail_plugin::send_mail_plugin() {}
        send_mail_plugin::~send_mail_plugin() {}
        void send_mail_plugin::set_program_options(options_description& cli,
                                                   options_description& cfg) {
            cfg.add_options()(
                "mail-login-user",
                boost::program_options::value<string>()->default_value("test@126.com"),
                "the login user of send mail.")(
                "mail-login-pass", boost::program_options::value<string>()->default_value("123456"),
                "the login user password of send mail.")(
                "mail-server-host",
                boost::program_options::value<string>()->default_value("smtp.126.com"),
                "the mail server host.")(
                "mail-server-port", boost::program_options::value<string>()->default_value("25"),
                "he mail server port.");
        }
