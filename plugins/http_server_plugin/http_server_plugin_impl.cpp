...
namespace hb {
    namespace plugin {
        ...
        std::shared_ptr<ssl::context> http_server_plugin_impl::load_ssl_cert() {
            ...
            ctx->set_password_callback([self](std::size_t, ssl::context_base::password_purpose) {
                return self->http_options_.private_password.c_str();
            });
            auto config_path = app().config_dir();
            ctx->use_certificate_chain_file(
                (config_path / http_options_.certificate_file).string());
            ctx->use_private_key_file((config_path / http_options_.private_file).string(),
                                      ssl::context::pem);
        ...
        void http_server_plugin_impl::start_http_server(std::shared_ptr<net::io_context> ioc) {
            ...
            l->run();
            log_info << "http server ip: " << http_options_.host << " port: " << http_options_.port
                     << " starting!";
        }
        void http_server_plugin_impl::start_https_server(std::shared_ptr<net::io_context> ioc) {
            ...
            l->set_expires_seconds(http_options_.expires_seonds);
            l->run();
            log_info << "https server ip: " << http_options_.host << " port: " << http_options_.port
                     << " starting!";
        }
