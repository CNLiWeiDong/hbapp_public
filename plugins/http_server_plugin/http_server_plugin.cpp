...
namespace hb {
    namespace plugin {
        ...
        void http_server_plugin::set_program_options(options_description& cli,
            ...
            cfg.add_options()("http-host",
                              boost::program_options::value<string>()->default_value("0.0.0.0"),
                              "the http server ip")(
                "http-port", boost::program_options::value<int>()->default_value(8888),
                "the http server port")(
                "http-body-size", bpo::value<uint64_t>()->default_value(1024 * 1024),
                "The maximum body size in bytes allowed for incoming requests")(
                "http-expires-seconds", bpo::value<uint32_t>()->default_value(30),
                "The connect keep live seconds time")
                // ("http-enable-path", bpo::value<vector<string>>(), "The connect keep live seconds
                // time")
                ("https-enable", boost::program_options::value<bool>()->default_value(false),
        ...
        void http_server_plugin::plugin_initialize(const variables_map& options) {
            ...
            if (options.at("https-enable").as<bool>()
                && (!(options.count("https-certificate-file")
                      && options.at("https-certificate-file").as<string>().length())
                    || !(options.count("https-private-file")
                         && options.at("https-private-file").as<string>().length())
                    || !(options.count("https-dhparam-file")
                         && options.at("https-dhparam-file").as<string>().length()))) {
                http_server_exception e;
                e.msg("https server need certificate-file and private-file in config-dir path!");
                hb_throw(e);
            }
            string certificate;
            if (options.count("https-certificate-file")
                && options.at("https-certificate-file").as<string>().length())
                certificate = options.at("https-certificate-file").as<string>();
            string dhparam;
            if (options.count("https-dhparam-file")
                && options.at("https-dhparam-file").as<string>().length())
                dhparam = options.at("https-dhparam-file").as<string>();
            string private_key;
            if (options.count("https-private-file")
                && options.at("https-private-file").as<string>().length())
