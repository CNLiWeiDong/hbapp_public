...
namespace hb {
    namespace plugin {
        log_server_plugin_impl::log_server_plugin_impl(const int &port) {
            socket_
                = make_shared<udp::socket>(app().get_io_service(), udp::endpoint(udp::v4(), port));
        }
        log_server_plugin_impl::~log_server_plugin_impl() {}
        void log_server_plugin_impl::do_receive() {
            socket_->async_receive_from(
                boost::asio::buffer(read_data_, max_length), sender_endpoint_,
        ...
        void log_server_plugin_impl::start() { do_receive(); }
